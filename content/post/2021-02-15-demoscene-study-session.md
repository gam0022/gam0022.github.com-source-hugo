...
- 質問2: シェーダーの数と行数について
    ...
    - グラフィックス用のシェーダーは最大（宇宙空間のレイマーチング）で700行、最小（Bloomのポストエフェクト）で10行ほど。用途によって幅があるが、レイマーチング用のシェーダーだと400行前後くらい。
- 質問3: ディレクションについて
    - 制作前にBPMは決めていた
    - 方向性は絵が先行
    - 尺については音楽が先行
...
最後のTokyoDemoFestは2018年の12月なので、もう2年以上も前なんですね。時間が経つのは早いです。

discord上の懇親会では「どうすればライブコーディングが普及できるのか？一般人でも理解できるような実況が必要という仮説」「物理的な会場のクラブの体験とVRの違い」など興味深いお話を聞けて面白かったです。

素晴らしいイベントを企画してくださったTDFのオーガナイザーのみなさん、ありがとうございました！
