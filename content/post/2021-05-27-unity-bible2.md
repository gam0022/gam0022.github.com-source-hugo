...
私は6年前にレイマーチングと出会い、レイマーチングの面白さに魅せられ、レイマーチングについて情報を発信してきました。

6年前の世界では、レイマーチングは一部の変態シェーダー使いの中だけで使われるニッチな技術というのが一般的な認識だったと思います。
固定長のレイマーチングがボリュームレンダリングの手法としてゲームで使われる事例はありましたが、距離関数の可視化手法としてのスフィアトレーシング（距離関数をステップにしたレイマーチング）がプロダクションで利用された事例はほとんど無かったと思います。

現在ではレイマーチングは[ゲーム](https://www.famitsu.com/news/201808/23162812.html)や[アニメ](http://cgworld.jp/feature/201602-kado01-cgw211.html)や[映画](https://cgworld.jp/feature/202001-cgwcc-hw.html)などのプロダクションで採用される事例が増え、実用的な技術として認識されつつあるというのが私の印象です。
レイマーチングの弱点はGPU負荷の高さですが、今後のGPUの性能向上により状況が改善し、さらに普及していくと予想しています。

Unityの一般ユーザー向けの書籍でレイマーチングについて解説をする機会をいただけたことをとても光栄に思います。
レイマーチングの普及を後押しできる千載一遇のチャンスなので、たくさんのUnity学習者をシェーダーの沼に引きずり込んで、レイマーチングを民主化したいです！

嬉しいことに現在ではレイマーチング入門の日本語の情報もかなり充実してきましたが、その次のステップの情報が不足していると個人的には感じていました。
本書ではレイマーチングの中級者向けの情報として、距離関数によるモデリングや本格的なライティングまで解説をしました。
レイマーチングで球体のレンダリングはできたけど、その次のステップが分からないという方にはぜひ読んでいただきたいです。

本書の定価8580円は気軽に購入しづらいと思いますが、「フルカラーの1112ページ、扱うテーマの広さ、得られる情報の密度」などを踏まえると妥当（むしろお買い得）な値段だと思います。
