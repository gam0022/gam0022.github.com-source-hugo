+++
slug = "unity-bible2"
date = "2021-05-31T10:30:19+09:00"
...
このセクションでは「レイマーチング」という手法により、グラフィックスを描画する仕組みを学びます。
...
このセクションでは、Unityの[Timeline](https://docs.unity3d.com/ja/2019.2/Manual/TimelineOverview.html)を利用した映像作品の制作について解説します。
...
サンプルプロジェクトは、筆者のGitHubで公開しています。サンプルシーンはチュートリアル形式で、一歩ずつレベルアップしながら理解を深められるようになっています。
...
裏話になりますが、**デモのメイキングを書籍として出版する** という試みに今回はチャレンジしました。
...
私は6年ほど前からレイマーチングについて情報を発信してきました。

6年前の世界では、レイマーチングは一部の変態シェーダー使いの中だけで使われるニッチな技術という扱いを受けていた印象でした。
固定長のレイマーチングがボリュームレンダリングの手法としてゲームで使われる事例はありましたが、距離関数の可視化手法としてスフィアトレーシング（距離関数をステップにしたレイマーチング）がプロダクションで利用された事例は自分の観測範囲では無かったと思います。

現在ではレイマーチングはゲームや映画などのプロダクションで採用される事例が増え、実用的な技術として認識されるようになりました。
今後のハードウェアの性能向上によりさらに普及していくと予想しています。

Unityの一般ユーザー向けの書籍でレイマーチングについて解説をする機会をいただけたことは、レイマーチングの普及を後押しできる千載一遇のチャンスが来たと捉えています。

たくさんのUnity学習者をシェーダーの沼に引きずり込んで、レイマーチングを民主化したいです！

ありがたいことに、レイマーチング入門の記事や情報はかなり充実してきましたが、その次のステップの情報が不足していると個人的には感じていました。
今回はレイマーチングの中級者向けの情報として、距離関数によるモデリングや本格的なライティングまで解説をしました。
レイマーチングを触って球体のレンダリングはできたけど、その次のステップが分からないという方にはぜひ読んでいただきたいです。

本書の定価8580円は流石に購入を躊躇する値段だと思いますが、「フルカラーの1112ページ、扱うテーマの広さ、得られる情報の密度」などを踏まえると妥当（むしろお買い得）な値段だと思います。

ちなみに初版部数はAmazonの予約数をもとに決まるそうなので、ぜひ[Amazonから予約](https://amzn.to/3oe2agf)をお願いします！
