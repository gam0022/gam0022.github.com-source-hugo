...
すべて平面として衝突判定を行い、ノーマルマップだけで海面のように見せています。
...
また、簡単な文字の出現と消滅のエフェクトを、シェーダーの一部を改造して実装しました。
この演出に関する解説を[Unity #2 Advent Calendar 2018](https://qiita.com/advent-calendar/2018/unity2)の19日目の記事で公開する予定です。
...
ワームホールの中身だけ別の世界が表示されている演出には戦闘機と同じObject Space Raymarchingの仕組みを利用しました。
...
カメラの原点からレイを進めると、別世界が3Dの立体映像のように飛び出してしまうという罠にハマってしまいました。
