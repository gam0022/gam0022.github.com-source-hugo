...
- コードの自動フォーマットをかけると、圧縮効率が向上してファイルサイズが減る
...
製作のイテレーションを高速化するため、個人的に欲しいエディタ機能は一通り実装しました。

- 再生位置のシーク機能
    - 再生・一時停止・停止・フレーム単位のコマ送り
- 時間やシークバーの表示単位の「秒/ビート」の切り替え
    - ビート単位で演出をつけることが多いので、個人的にはビート単位の方が便利
...
さきほどのスクリーンショットの右上の拡大ですが、インスペクタ上ではこうなります。
...
インスペクタ上で値の当たりをつけた後に、パラメータのアニメーションを[TypeScriptのコード](https://github.com/gam0022/resimulated/blob/master/src/index.common.ts#L142-L569)に落とし込むワークフローにしました。

dat.GUIだと細かい値が表示表は丸められてしまうのに悩まされました。値はsessionStorageに保存しているので、それを参照して細かい値を確認しましたが、根本的に対応したい気はしています。
...
pal 関数の使い方は簡単で、`a, b, c, d` に任意の値を指定して、`t` を変化させればグラデーションを生成できます。
...
今回のRevisionためにプログラミングとGLSLを習得して、コーディングのみで作曲していただいたさだきちさんには感謝しかありません。

かなり無茶なお願いだったにも関わらず、かっこいいトランスミュージックを作っていただきました！ありがとうございます！

また、私自身もWebフロントエンドは久しぶりだったので、今回は新しい技術を学ぶ良い機会となりました。

昔はjQueryが必要だったDOMのセレクターやHTTPアクセスは、標準API（[querySelector](https://developer.mozilla.org/ja/docs/Web/API/Document/querySelector), [Fetch](https://developer.mozilla.org/ja/docs/Web/API/Fetch_API/Using_Fetch)）で普通にできるようになっていて驚きました。

あとは演算子オーバーロードさえあれば、3Dプログラミングでも十分に実用的な言語になるのではないかと個人的には思うので、早く対応してほしいです。

<blockquote class="twitter-tweet" data-conversation="none"><p lang="ja" dir="ltr">TypeScript、演算子オーバーロードさえあれば、メインウェポンになりえそうな言語だと思いました…</p>&mdash; がむ / encoder killer (@gam0022) <a href="https://twitter.com/gam0022/status/1249693614260678656?ref_src=twsrc%5Etfw">April 13, 2020</a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>

なんと世界1位になってしまいましたが、今回の結果に慢心せず、logicomaやLJやConspiracyやMonadやPolarityレベルのかっこいい64K Introを作れるように引き続き努力していきたいです。