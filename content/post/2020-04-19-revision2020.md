...
後半の宇宙空間とグリーティングのシーンでは、宇宙空間はレイマーチング、惑星の上のグリーティングの文字はAABBとして解析的な衝突判定をしたハイブリッドな衝突判定をしています。
...
パターン（2小節分のノートナンバーの並び）、パターンの展開 を配列で指定できるようにて、4種類の音の長さに対応しました。
...
GLSLのintは32bitなので、1つのint配列に16分音符は2つ、8分音符が1つ入るような設計です。
...
`SEQUENCER` は、時間、パターンの配列、展開の配列、オシレーター関数を指定することで、パートごとの波形を生成して `vec2 ret` に代入する関数マクロです。

GLSLでは関数を引数とするような高階関数は実現できませんが、関数マクロで擬似的に実現しました。

`SEQUENCER` の実装はかなり汚いのですが、`bass1` などのパートごとの関数は非プログラマーでも実装できるような、分かりやすい仕組みを提供できたのではないかと自負しています。
