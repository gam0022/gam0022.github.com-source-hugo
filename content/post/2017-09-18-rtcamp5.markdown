...
今年は16人中9位だったので、去年の13人中10位よりは進歩しました！
...
パストレーシングはレンダリング方程式をモンテカルロ積分する手法でしたね。

つまり、モンテカルロ積分のサンプリングを効率化すれば、より少ないサンプリング数でノイズの少ない収束した結果を得ることができます。
...
この戦略はサンプリング数を事前に決定する必要があるため、サンプリング数の調整が必要でした。
サンプリング数を大きくしすぎると4:33の制限時間をオーバーしすぎる心配があり、
逆に小さくしすぎると計算リソースを使い切れなくなってしまいます。

今回のようにレンダリング時間がルールで決まっている場合は、あまり良い戦略ではありません。
...
これでサンプリング数をスペックに応じて動的に調整できるようになりました。

しかし、この変更によって5%くらいパフォーマンスが低下しました。
同じサンプリング数で比較すると、計算時間が5%程度増加しました。
テクスチャをたくさん使ったシーンなので、キャッシュのヒット率が低下したのかなと思っています。
...
- Webブラウザ上でも動かせる
  - Mollizaが開発していることもあり、C++を除いて唯一のWebAssemblyの出力をサポートする言語
...
正直に言うと、はじめはRustの学習コストは高いと感じました。
...
合宿のセミナーや雑談などでレンダリング分野の最先端で活躍されている方々にレイトレの知見をもらってモチベーションが高まりました。
セミナーでは大垣さんの20年間レンダラーを開発しているうちに考えたこと、
うしおさんのファーのレンダリング、Otsuさんの最新のPM/MCMC系手法のまとめについて聞きました。
また、他の参加者にもNext Event Estimationの疑問点、双方向パストレーシングについてを教えていただきました。

宿の料理は美味しかったですし、温泉から見える夕暮れの諏訪湖も綺麗だったので、観光としても良かったです。
