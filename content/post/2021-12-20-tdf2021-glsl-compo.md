...
そもそもGLSLシェーダー、つまり **プログラミングのソースコードだけで映像をつくる** 行程を一般的には想像しずらいかもしれません。
...
sinから滑らかなカーブを得て、それをfloorで階段状に離散化いるだけです。
...
```c
// カメラワーク制御の実装例
TL(beat, 4. * 8.) setCamera(vec4(600, 250. + t * 3., 600, 243. - t * 6.), 3.);//    0ビート から 4 * 8ビート目 までの処理
else TL(beat, 4. * 10.) setCamera(vec4(600, 307, 600, 44. + t * 4.), 3.);//         4 * 8ビート  から 4 * 10ビート目 までの処理
else TL(beat, 4. * 12.) setCamera(vec4(494, 322, 695, 216), 2.4 + 0.2 * t);//       4 * 10ビート から 4 * 12ビート目 までの処理
else TL(beat, 4. * 14.) setCamera(vec4(600, 481. + 10. * t, 600, 59), 3.);//        4 * 12ビート から 4 * 14ビート目 までの処理
```
...
[Traveler 2](https://nanka.hateblo.jp/entry/2018/12/13/080322)やAlien Spaceshipのような短いグラフィックスではなく長尺のデモっぽい作品をGLSL Graphics Compoを出すことで、もし他の参加者が萎縮してしまったらとても不本意な気持ちです。

個人的にはGLSL Graphics Compoは順位や周りを気にせず、どんどんエントリーする人が増えたら嬉しいです。
...
Day2のYouTubeの視聴回数が3000回を超えているので、例年のオフラインパーティよりもたくさんの人に見てもらたといったオンラインのメリットも感じました。
