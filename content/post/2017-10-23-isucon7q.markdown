...
slug = "isucon7q"
date = "2017-10-23T22:05:05+09:00"

+++

ISUCONはお題となるWebサービスを決められたレギュレーションの中で限界まで高速化を図るチューニングバトルです。

会社の同期と2人チーム「アロマゲドン」を結成して、ISUCON7予選に参加しました。
ちなみに「アロマゲドン」は、プリパラというアニメに出てくるアイドルユニットです。
白玉みかんは私（[@gam0022](https://twitter.com/gam0022)）で、黒須あろまは[@kanetaaaaa](https://twitter.com/kanetaaaaa)でした。

[407チーム中81位で、最終スコアは46,461](http://isucon.net/archives/50961437.html)でした。

~~上位18位まで本戦出場だったので、数字をひっくり返せば81が18になって本戦出場できたかもしれません。~~

アプリのチューニングはある程度はできたのですが、`GET /icons`の帯域問題に対処できなかったのが敗因でした。

敗者の記事を読む人はいないでしょうが、来年の自分に向けて感想を残そうと思います。
...
私は過去に1度だけISUCONに参加した経験はあったのですが、あろまはISUCONの経験はありませんでした。

あろまには、ISUCON5の本戦用に私が用意したチートシートや[ISUCON夏期講習](http://isucon.net/archives/50648750.html)のスライドを読んで勉強してもらいました。

私は技術書典の原稿執筆と重なってしまい、ほとんど練習時間を確保できずに当日を迎えました。

## 過去問の対策

休日に会社で集まって、役割分担を意識しながらチームで問題を解く練習をしました。
[pixiv ISUCON](https://github.com/catatsuy/private-isu)がEC2のAMIが公開されており、環境構築が簡単だったので、これを使って練習しました。

私はISUCON5の予選に参加した経験があり、ISUCON4の予選も練習したことがあったので、今回は初見のpixiv ISUCONしか練習しませんでした。

あろまはGoの経験があったので、はじめはGoで練習したのですが、私がGoの経験が無さすぎたので、
最終的には2人ともある程度読み書きができるRubyで問題を解くことにしました。

pixiv ISUCONとISUCON7の予選は非常に似ていまいた。
...
練習の方向性は間違っていなかったので、詰が甘かったことを本当に後悔しています。
...
<iframe src="/images/posts/2017-10-23-isucon7q/summary_log_init.html" style="width: 100%; height: 400px;"></iframe>

ISUCON用のslackのグループを作って、上記のサマライズの結果をslackに投稿する仕組みを作りました。
...
- みかん : `/home/isucon/isubata/webapp` と `/etc` をGit管理、リポジトリ作成
- あろま : MySQLのユーザ名とパスワードを調査
  - 事前に用意していたMySQL関係のaliasでは、ユーザ名をrootでパスワードなしを前提としていて、動かずに困っていたが、これで解決
- あろま : apt-get update
- あろま : vimやnetdataのインストール
- あろま : MySQLのバックアップ
...
- みかん : imageテーブルの画像をファイルとして読み書きしてnginxで配信するように変更
  - pixiv ISUCONで予習済みだったため、すんなりとできた
- あろま : imageテーブルをDrop
- あろま : cssやjsなどの静的ファイルをnginxで配信。さらにgzip_staticで帯域の節約
- あろま : `GET /icons`で304を返せるようにnginxのCache-Controlの設定
  - 多くのチームが付けずに苦しめられた `Cache-Control: public` をつけてくれた（超絶ナイス）
- あろま : stackprofというRubyのプロファイラを導入
- みかん : nginxのworker_connections 1024 にしたら、スコアが6000→3000に下がったので、リバート
- みかん : `GET /message`が時々9秒など異常に遅いときがあるのに気がつく
  - N+1問題を解消
...
事前の練習や準備は限られた時間にしては頑張ったと思うのですが、残念な結果に終わってしまいました。

他にもレギュレーションをちゃんと読まなかった点も反省です。
スコアに関して、かなり重大なポイントを見落としていました。
...
[ISUCON7 予選問題の公開と解説](http://isucon.net/archives/50961706.html)に以下のようにあります。
...
初期状態で何件のメッセージを返せていたのか不明ですが、1回の`GET /message`の点数はおよそ1/5に落ちていたようです。
...
これは超個人的な意見ですが、複数台構成や帯域制限のある構成は再現が困難で、試合後の復習ができないのでやめて欲しいと思いました。
希望を言えば、EC2のAMIが公開されていて、特定のインスタンプタイプでインスタンスを作ればすぐに環境を再現できるような問題が嬉しいです。

愚痴っぽくなってしまいましたが、スコアを競うゲームとして純粋に楽しめました。
ISUCONの練習や予選を通して得たものは多かったので、来年も是非参加したいです。

運営の方々、楽しいイベントをありがとうございました。
今は本戦の準備で大忙しだと思いますが、頑張ってください！
