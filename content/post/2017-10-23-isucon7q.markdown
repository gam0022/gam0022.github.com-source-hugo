...
ステータスコードごとによる集計によると304が最多でしたので、キャッシュコントロールは正しく効いていそうでした。
...
24765 status:304
 ...
 そうすれば、画像のアップロード直後に304を返せるので、さらに`/icons`の帯域を節約できたのではないかと思います。
...
短期間にしては事前準備に力を入れられたものの、残念な結果に終わってしまいました。
