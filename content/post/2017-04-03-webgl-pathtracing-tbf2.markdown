...
今年はWebGLによるGPUパストレーシングについて書きました。
去年12月のWebGLのアドベントカレンダーのQiita記事とテーマは同じですが、内容を大幅に加筆・変更しています。
...
開発したWebGLパストレーシングはブラウザ上で動かせるので、良ければ触っていただけると嬉しいです。
...
執筆にあたってコードを見返したところ、様々なバグを見つけたので、原稿と平行してバグ修正をしました。
睡眠時間と引き換えにバグを取り除いたことで、レンダリングの品質を大きく改善できました！
...
![コーネルボックス](/images/posts/2017-04-03-webgl-pathtracing/cornellbox.png)
...
![テーブル](/images/posts/2017-04-03-webgl-pathtracing/table.png)
...
自分の章はニッチすぎると思いますが、明らかに値段以上の量と質のあるお得な本になっていると思います。
こちらが本の内容です。それぞれの技術のスペシャリストが全力で執筆しています。
...
この記事はパストレーシングが分かっている人の向けに書いたので細かい解説は省きましたが、TechBoosterのWeb本ではパストレーシングについて基礎から解説しました。
[Qiitaの記事](http://qiita.com/gam0022/items/18bb3612d7bdb6f4360a)では書ききれなかった基礎的な部分が中心となっています。
レイトレーシングは分かっているけど、パストレーシングは難しそうだと思っている方にオススメです。

今回の記事では本の内容とは関係ない実装の修正の裏話のみとなってしまったので、後日、本の内容に触れた宣伝用の記事を投稿する予定です。
