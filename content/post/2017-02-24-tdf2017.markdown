...
2017年2月18日(土)〜19日(日)の2日間開催された[Tokyo Demo Fest 2017](http://tokyodemofest.jp/2017/)に参加しました。
...
（去年と全く同じ部門で全く同じ順位だったので、順位を上げられなかったのは残念ではあります）
...
負荷の高い処理を多用した作品なので、GPUを搭載したPCじゃないと、フルスクリーンで動作させるのは厳しいです。
目安としては、自宅のGTX1070を積んだPCだと、快適に動作します。
...
余談ですが、今ではレイマーチングはすっかりデモシーン界隈で定着された手法になってしまって、GLSL Compoの参加者のほとんどが利用されていました。
たぶん10人中8人はレイマーチングだったのではないでしょうか。
...
dTreeの木のような形をうまく調整し、Z軸方向に6回回転させるfoldを適用すると「Fusioned Bismuth」に登場した雪の結晶のような形状を得られます。
...
また、任意の法線を持った面に対する一般化されたfoldがSyntopiaに紹介されています。興味のある方は、見てみると良いでしょう。
...
ソフトシャドウは昔に覚えた方法を使いました。良ければ[スライド](https://speakerdeck.com/gam0022/reimatingudesohutosiyadou)を見て下さい。
...
そのままランバートシェーディングを行うと、陰影がはっきりしすぎるので、`0.5 * diffuse + 0.5` という式で補正しました。
このテクニックはハーフランバートという名前が付いているそうですね。
...
複数の頂点をシーン上に配置して、時間軸をパラメータにしてブレンドしました。
なかなか思った通りにカメラを動かせず、時間や手間がかかってしまいました…

ところで、Combined PC Demo Compoで1位の「Shift」を生み出した[@FMS_Cat](https://twitter.com/FMS_Cat)さんは、TDFのためにAutomatonというツールを開発されていました。カメラの動きもGUI上で制御できるような仕組みのようです。流石ですよね。
...
TDFについて、他にも書きたいことはたくさんあるのですが、まとめきれないので自分の作品の紹介のみに留めさせていただきます。

とにかくTDFは最高のイベントでした。

どのCompoを見ても新しい発見だらけでしたし、普段は絶対にできないレイマーチングについての深い議論もできました。

デモシーンは面白い文化だと思うので、もっとたくさんの人が興味を持って欲しいと思いますね。
